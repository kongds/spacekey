...
menu_hide = False
def set_menu_visible(hide=False):
    global menu_hide
    if hide:
        if not menu_hide:
...
def spaceit(key_char):
    ...
    if key_char in app_key_map:
        ...
        if key_char != 'e' and not os.path.exists('/Users/royokong/.eaf-showing'):
...
while not kill_self:
    ...
    time.sleep(0.2)
    if len(subprocess.check_output('m1ddc display list'.split())) == 0:
